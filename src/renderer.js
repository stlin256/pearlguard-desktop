...
function renderShell(pageHtml) {
  ...
  document.getElementById('app').innerHTML = `<aside class="sidebar">
    ...
    <div class="side-status">${statusPill(appState.bootstrap?.mode === 'demo' ? t('status.demo') : t('status.localMode'), appState.bootstrap?.mode === 'demo' ? 'warn' : 'safe')}<span>${escapeHtml(t('status.noBroadcast'))}</span></div>
  </aside>
  <section class="workspace">
    <header class="topbar"><div><p class="eyebrow">${escapeHtml(t('app.eyebrow'))}</p><h1>${escapeHtml(t(`page.${appState.activePage}`))}</h1></div>
      <div class="top-actions"><select id="localeSelect" aria-label="${escapeHtml(t('settings.language'))}">${supportedLocales.map((locale) => `<option value="${locale}" ${locale === appState.locale ? 'selected' : ''}>${localeNames[locale]}</option>`).join('')}</select><button class="ghost-button" id="syncPoolsTop">${escapeHtml(t('action.syncPools'))}</button></div></header>
  ...
  document.getElementById('localeSelect').addEventListener('change', async (event) => { appState.locale = event.target.value; localStorage.setItem('pearlguard.locale', appState.locale); appState.messages = await loadMessages(appState.locale); render(); });
  document.getElementById('syncPoolsTop').addEventListener('click', syncPoolsAndRender);
}

function setupPanel() {
  if (appState.bootstrap?.mode === 'demo' || hasRealData()) return '';
  return `<section class="panel action-panel setup-panel"><div><h2>${escapeHtml(t('dashboard.setupTitle'))}</h2><p>${escapeHtml(t('dashboard.setupCopy'))}</p></div><div class="button-row"><button class="primary-button" id="refreshWalletTop">${escapeHtml(t('action.refreshWallet'))}</button><button class="ghost-button" id="importCsvTop">${escapeHtml(t('action.importCsv'))}</button><button class="ghost-button" id="openConfigTop">${escapeHtml(t('action.openConfigFolder'))}</button></div></section>`;
}
...
function renderDashboard() {
  ...
  const recentAudit = getAuditEvents().slice(-4).reverse();
...
function renderMonitor() {
  ...
  return `<section class="panel action-panel"><div><h2>${escapeHtml(t('monitor.title'))}</h2><p>${escapeHtml(t('monitor.copy'))}</p></div><div class="button-row"><button class="primary-button" id="refreshWalletPage">${escapeHtml(t('action.refreshWallet'))}</button><button class="ghost-button" id="dryRunCheck">${escapeHtml(t('monitor.runDryCheck'))}</button></div></section>
  ...
  <section class="panel"><div class="panel-header"><h3>${escapeHtml(t('monitor.latestDecision'))}</h3>${statusPill(t('status.readOnly'), 'safe')}</div><pre class="decision-box">${escapeHtml(JSON.stringify(dry || { message: t('monitor.noDecision') }, null, 2))}</pre></section>`;
}
...
function verifyCurve(snapshots, wallet) {
  ...
  return [
    ...
    { tone: 'safe', title: t('verify.feeTitle'), message: t('verify.feeMessage') }
  ];
}
...
function renderPoolsPage() {
  ...
  return `<section class="panel action-panel"><div><h2>${escapeHtml(t('pools.title'))}</h2><p>${escapeHtml(t('pools.copy'))}</p></div><button class="primary-button" id="syncPoolsPage">${escapeHtml(t('action.syncPools'))}</button></section><section class="metric-grid">${metricCard(t('pools.catalog'), String(catalog.length), t('pools.catalogHint'))}${metricCard(t('pools.reachable'), String(pools.filter((p) => p.reachable).length), t('pools.reachableHint'))}${metricCard(t('pools.lastSync'), fmtTime(appState.poolSync?.timestamp), appState.poolSync?.mode || 'local')}${metricCard(t('pools.adapters'), '5', 'zpool, yiimp, miningcore, nomp, generic')}</section><section class="panel">${renderPoolRows(pools)}</section>`;
}
...
function renderSettings() {
  const paths = appState.bootstrap.paths || {};
  return `<section class="panel settings-panel"><h2>${escapeHtml(t('settings.title'))}</h2><div class="settings-grid"><div><span>${escapeHtml(t('settings.language'))}</span><strong>${escapeHtml(localeNames[appState.locale])}</strong></div><div><span>${escapeHtml(t('settings.platform'))}</span><strong>${escapeHtml(appState.bootstrap.platform)}</strong></div><div><span>${escapeHtml(t('settings.version'))}</span><strong>${escapeHtml(appState.bootstrap.version)}</strong></div><div><span>${escapeHtml(t('settings.repo'))}</span><strong>${escapeHtml(appState.bootstrap.repoUrl)}</strong></div></div></section><section class="panel"><div class="panel-header"><h3>${escapeHtml(t('settings.localFiles'))}</h3><button class="ghost-button" id="openConfigPage">${escapeHtml(t('action.openConfigFolder'))}</button></div><ul class="plain-list"><li><code>${escapeHtml(paths.walletConfig || 'wallet.config.json')}</code> — ${escapeHtml(t('settings.walletLocal'))}</li><li><code>${escapeHtml(paths.poolsConfig || 'pools.local.json')}</code> — ${escapeHtml(t('settings.poolLocal'))}</li><li><code>${escapeHtml(paths.stateFile || 'pearlguard-state.json')}</code> — ${escapeHtml(t('settings.runtimeLocal'))}</li></ul></section>`;
}
...
function render() {
  ...
  document.querySelectorAll('#refreshWalletTop,#refreshWalletPage').forEach((button) => button.addEventListener('click', refreshWallet));
  document.querySelectorAll('#importCsvTop,#importCsvPage').forEach((button) => button.addEventListener('click', importCsv));
  document.querySelectorAll('#openConfigTop,#openConfigPage').forEach((button) => button.addEventListener('click', () => window.pearlGuard.openConfigFolder()));
...
window.__pearlguardSelfTest = async () => {
  ...
  return {
    ok: Boolean(document.querySelector('.app-shell') && appState.bootstrap.mode === 'local' && poolSync.observations.length >= 1 && labels.includes(t('nav.dashboard'))),
